...
public class HttpRequest implements HttpServletRequest{
				...
				if(count == 0){
					...
					if(m.find()){
						...
						if(!headers.containsKey(header.toLowerCase())){
							headers.put(header.toLowerCase(), new ArrayList<String>());
						}
	...
	private void parseBody() throws IOException{
		if(headers.containsKey("content-type")){
			if(request_method.equals("POST")&&headers.get("content-type").get(0).contains("application/x-www-form-urlencoded")){
				String s;
	...
	private void parseURL(){
	    ...
	    if (m.find()) {
	    	...
	    	path = m.group(2);
	    	query = m.group(3);
	    	if(query!=null) query = query.substring(1);
	    	parseQuery(query);
//	    	fragment = m.group(4);
	    }
	    else{
	...
	String getExtension(){
		if(isValidFile()){
			...
			if(index < filename.length()-1) return filename.substring(index+1);
		}
		return null;
	}
		...
		for(String s:queries.keySet()){
			if(s.equals("JSESSIONID")){
				...
				if(session!=null){
					...
					scheduledExecutorService.schedule(session.sessionTask, session.getMaxInactiveInterval(),TimeUnit.SECONDS);
					return session;
				}
			}
		}
	...
	public boolean isRequestedSessionIdValid() {
		return isRequestedSessionIdFromCookie()||isRequestedSessionIdFromUrl();
	}
